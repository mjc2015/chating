...
class MyServer(socketserver.BaseRequestHandler):
    ...
    def handle(self):
        ...
        global connLst
        global groupLst
        consql = pymysql.connect(host='127.0.0.1', user='root', password='123456', db='chat', charset='utf8')
        cur = consql.cursor()
        while not userIn:
            conn = self.request
            data = conn.recv(1024)
        ...
        while True:
            ...
            if dataobj['type'] == 'ag' and userIn:  #add a group
            ...
            if dataobj['type'] == 'eg' and userIn:   #entern a group
                groupName = dataobj['groupName']
                ret = 'eg1'
            ...
            if len(connLst) > 1:
                sendok = False
                if dataobj['type'] == 'cg':   # chat with group
                    print('group',data)
                    for obj in groupLst:
